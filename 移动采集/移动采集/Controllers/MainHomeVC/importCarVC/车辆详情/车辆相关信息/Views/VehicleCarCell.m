    ...
    if (_imagelists && _imagelists.count > 0) {
        ...
        for (int i = 0;i < [_arr_view count]; i++) {
            ...
            if (i == _arr_view.count - 1 ) {
                ...
                [t_button_last autoPinEdge:ALEdgeBottom toEdge:ALEdgeBottom ofView:self.contentView withOffset:-15.0];
            }
        }
        
    }else{
        
        [_lb_vehicleImgList autoPinEdgeToSuperviewEdge:ALEdgeBottom withInset:15];
...
- (IBAction)btnTagAction:(id)sender{
    ...
    VehicleCarInfoMoreVC *vc_target = (VehicleCarInfoMoreVC *)[ShareFun findViewController:self withClass:[VehicleCarInfoMoreVC class]];
