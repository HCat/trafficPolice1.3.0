...
@implementation TakeOutSaveParam
...
@implementation TakeOutSaveManger

//请求的url，不包括域名`域名通过YTKNetworkConfig配置`
- (NSString *)requestUrl{
    
    return URL_TAKEOUT_SAVE;
}
...
@implementation TakeOutTypeListManger

- (NSString *)requestUrl
{
    return URL_TAKEOUT_TYPELIST;
}
...
@implementation TakeOutSubmitTempReportParam

//黑名单，不被转换
+ (NSArray *)modelPropertyBlacklist {
    return @[@"files",@"certFileInfo"];
}
...
@implementation TakeOutSubmitTempReportManger

//请求的url，不包括域名`域名通过YTKNetworkConfig配置`
- (NSString *)requestUrl{
    
    return URL_TAKEOUT_SUBMITTEMPREPORT;
}
...
@implementation TakeOutCompanyListManger

- (NSString *)requestUrl
{
    return URL_TAKEOUT_COMPANYLIST;
}

- (NSArray < DeliveryCompanyModel *> *)list{
    
    if (self.responseModel) {
        _list = [NSArray modelArrayWithClass:[DeliveryCompanyModel class] json:self.responseJSONObject[@"data"]];
