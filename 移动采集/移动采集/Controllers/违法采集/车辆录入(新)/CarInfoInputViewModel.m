...
#import "CarInfoInputViewModel.h"
#import "CommonAPI.h"

@implementation CarInfoInputViewModel
...
- (instancetype)init{
    ...
    if (self = [super init]) {
        ...
        @weakify(self);
        [[RACSignal combineLatest:@[RACObserve(self.param, roadId), RACObserve(self, first),RACObserve(self.param, address),RACObserve(self.param, longitude),RACObserve(self.param, latitude),RACObserve(self.param, carNo)] reduce:^id (NSNumber * roadId,id first,NSString * address,NSNumber * longitude,NSNumber * latitude,NSString * carNo){
            return @(roadId && ![first isKindOfClass:[NSNull class]] && address.length > 0 && longitude && latitude && carNo.length > 0);
        }] subscribeNext:^(id x) {
            @strongify(self);
    ...
    if (_command_commit == nil) {
        _command_commit = [[RACCommand alloc] initWithSignalBlock:^RACSignal * _Nonnull(id  _Nullable input) {
            ...
            RACSignal * t_signal = [RACSignal createSignal:^RACDisposable * _Nullable(id<RACSubscriber>  _Nonnull subscriber) {
                ...
                self.param.type = @(ParkTypeCarInfoAdd);
                ...
                [manger startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest * _Nonnull request) {
                    ...
                    if (manger.responseModel.code == CODE_SUCCESS) {
                        ...
                        [LRShowHUD showError:manger.responseModel.msg duration:1.2f];
                    }
...
- (void)handleBeforeCommit{
    ...
    [[LocationStorage sharedDefault] setInforInput:model];
