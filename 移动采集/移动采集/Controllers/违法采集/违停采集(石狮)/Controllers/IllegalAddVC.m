    ...
    [self.viewModel.command_carNoSect.executionSignals.switchToLatest subscribeNext:^(id  _Nullable x) {
        ...
        if ([x isKindOfClass:[RACTuple class]]) {
            ...
            if ([code isEqualToNumber:@0]) {
                //超过90秒有一次采集记录，返回一次采集ID、采集时间，提示“同路段该车于yyyy-MM-dd已被拍照采集”，跳转至二次采集页面
...
-(void)handleBtnBackClicked{
    ...
    if (self.headView.param.addressRemark || self.headView.param.carNo || (self.viewModel.arr_upImages.count > 0 && ![self.viewModel.arr_upImages[0] isKindOfClass:[NSNull class]] && ![self.viewModel.arr_upImages[1] isKindOfClass:[NSNull class]])) {
