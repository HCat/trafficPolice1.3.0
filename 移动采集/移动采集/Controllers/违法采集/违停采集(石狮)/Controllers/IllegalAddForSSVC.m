...
#import "IllegalAddForSSViewModel.h"
...
@property (strong, nonatomic) IllegalAddForSSViewModel * viewModel;
...
@implementation IllegalAddForSSVC
...
- (instancetype)init{
    ...
    if (self = [super init]) {
        ...
        self.viewModel = [[IllegalAddForSSViewModel alloc] init];
    }
...
#pragma mark - 提交按钮事件
    [[self.btn_commit rac_signalForControlEvents:UIControlEventTouchUpInside ] subscribeNext:^(__kindof UIControl * _Nullable x) {
        ...
        if ([UserModel isPermissionForIllegalAdd] == NO) {
    ...
    [self.viewModel.command_carNoSecond.executionSignals.switchToLatest subscribeNext:^(id  _Nullable x) {
        ...
        if ([x isKindOfClass:[RACTuple class]]) {
            ...
            if ([code isEqualToNumber:@0]) {
                ...
                [self showAlertViewWithcontent:msg leftTitle:@"退出" rightTitle:@"确定" block:^(AlertViewActionType actionType) {
                    @strongify(self);
...
#pragma mark - 判断定位功能是否开启做相应的UI操作
    ...
    if ([LocationStorage sharedDefault].isIllegal == NO) {
        [self stopLocationAction:[LocationStorage sharedDefault].illegal];
    }
