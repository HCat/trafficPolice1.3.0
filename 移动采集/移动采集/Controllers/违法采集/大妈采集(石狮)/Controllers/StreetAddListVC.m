...
@interface StreetAddListVC ()<UITableViewDelegate,UITableViewDataSource>

@property (weak, nonatomic) IBOutlet LRBaseTableView *tableView;
@property(nonatomic,strong) StreetAddListViewModel * viewModel;
...
@implementation StreetAddListVC

- (instancetype)init{
    
    if (self = [super init]) {
        self.viewModel = [[StreetAddListViewModel alloc] init];
    }
...
- (void)configUI{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(illegalSuccess:) name:NOTIFICATION_STREETADDSS_SUCCESS object:nil];
...
- (void)bindViewModel{
    ...
    if ([UserModel isPermissionForStreetList]) {
...
- (IBAction)handleBtnSearchClicked:(id)sender {
    ...
    if ([UserModel isPermissionForStreetList] == NO) {
