...
@interface StreetAddForSSVC ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
...
@property (strong, nonatomic) StreetAddForSSViewModel * viewModel;
...
@implementation StreetAddForSSVC
...
- (instancetype)init{
    ...
    if (self = [super init]) {
        ...
        self.viewModel = [[StreetAddForSSViewModel alloc] init];
    }
...
#pragma mark - 检查违停采集是否拥有违停记录
    ...
    [self.viewModel.command_illegalRecord.executionSignals.switchToLatest subscribeNext:^(id  _Nullable x) {
        ...
        if ([x isKindOfClass:[RACTuple class]]) {
            RACTupleUnpack(NSNumber * code,StreetCarNoRecordReponse * reponse,NSString * msg) = x;
...
#pragma mark - 判断定位功能是否开启做相应的UI操作
    [RACObserve(self, btnType) subscribeNext:^(NSNumber * x) {
        ...
        if ([x boolValue]) {
            ...
            [self zx_rightClickedBlock:^(ZXNavItemBtn * _Nonnull btn) {
                ...
                [self stopLocationAction:[LocationStorage sharedDefault].street];
    ...
    if ([LocationStorage sharedDefault].isStreet == NO) {
        [self stopLocationAction:[LocationStorage sharedDefault].street];
    }
...
- (void)handleAfterCommitForUI{
    ...
    if (self.btnType != 1) {
        [self stopLocationAction:[LocationStorage sharedDefault].illegal];
    }
...
- (void)dealloc{
    NSLog(@"StreetAddForSSVC dealloc");
}
