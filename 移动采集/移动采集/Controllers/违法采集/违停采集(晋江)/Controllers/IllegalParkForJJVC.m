...
@interface IllegalParkForJJVC ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
...
@property (strong, nonatomic) IllegalParkForJJViewModel * viewModel;
...
@implementation IllegalParkForJJVC
...
- (instancetype)init{
    ...
    if (self = [super init]) {
        ...
        self.viewModel = [[IllegalParkForJJViewModel alloc] init];
...
#pragma mark - 提交按钮事件
    [[self.btn_commit rac_signalForControlEvents:UIControlEventTouchUpInside ] subscribeNext:^(__kindof UIControl * _Nullable x) {
        ...
        if ([UserModel isPermissionForIllegal] == NO) {
    ...
    [RACObserve(self, btnType) subscribeNext:^(NSNumber * x) {
        ...
        if ([x boolValue]) {
            ...
            [self zx_rightClickedBlock:^(ZXNavItemBtn * _Nonnull btn) {
                ...
                [[LocationStorage sharedDefault] closeLocation:ParkTypePark];
            ...
            [self zx_rightClickedBlock:^(ZXNavItemBtn * _Nonnull btn) {
                ...
                [[LocationStorage sharedDefault] startLocation:ParkTypePark];
    ...
    if (indexPath.row == self.viewModel.arr_upImages.count) {
        ...
        if ([self.viewModel.arr_upImages[0] isKindOfClass:[NSNull class]]) {
            ...
            [self showCameraWithType:3 withCutFinishBlock:^(ImageFileInfo *imageInfo, ImageFileInfo *cutImageInfo) {
                @strongify(self);
                [self.viewModel replaceUpImageItemToUpImagesWithImageInfo:imageInfo remark:@"违停照片" replaceIndex:0];
                [self.viewModel.command_identifyCarNo execute:imageInfo];
                [self.collectionView reloadData];
            }];
...
- (void)dealloc{
    NSLog(@"IllegalParkForJJVC dealloc");
}
