...
@interface NoPressTowardVC ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
...
@property (strong, nonatomic) NoPressTowardViewModel * viewModel;
...
@implementation NoPressTowardVC
...
- (instancetype)init{
    ...
    if (self = [super init]) {
        ...
        self.viewModel = [[NoPressTowardViewModel alloc] init];
...
#pragma mark - 提交按钮事件
    [[self.btn_commit rac_signalForControlEvents:UIControlEventTouchUpInside ] subscribeNext:^(__kindof UIControl * _Nullable x) {
        ...
        if ([UserModel isPermissionForIllegalReverseParking] == NO) {
    ...
    [RACObserve(self, btnType) subscribeNext:^(NSNumber * x) {
        ...
        if ([x boolValue]) {
            ...
            [self zx_rightClickedBlock:^(ZXNavItemBtn * _Nonnull btn) {
                ...
                [[LocationStorage sharedDefault] closeLocation:ParkTypeReversePark];
            ...
            [self zx_rightClickedBlock:^(ZXNavItemBtn * _Nonnull btn) {
                ...
                [[LocationStorage sharedDefault] startLocation:ParkTypeReversePark];
    ...
    for (int i = 0; i < [self.viewModel.arr_upImages count]; i++) {
        ...
        if ([self.viewModel.arr_upImages[i] isKindOfClass:[NSMutableDictionary class]]) {
            ...
            if ([t_str isEqualToString:[itemDic objectForKey:@"remarks"]]) {
                ...
                if ([t_str isEqualToString:@"车牌近照"] || [t_str isEqualToString:@"不按朝向照片"]) {
    ...
    if (indexPath.row == self.viewModel.arr_upImages.count) {
        ...
        if ([self.viewModel.arr_upImages[0] isKindOfClass:[NSNull class]]) {
            ...
            [self showCameraWithType:3 withCutFinishBlock:^(ImageFileInfo *imageInfo, ImageFileInfo *cutImageInfo) {
                @strongify(self);
                [self.viewModel replaceUpImageItemToUpImagesWithImageInfo:imageInfo remark:@"不按朝向照片" replaceIndex:0];
                [self.viewModel.command_identifyCarNo execute:imageInfo];
                [self.collectionView reloadData];
            }];
