    ...
    [self.command_dispose.executionSignals.switchToLatest subscribeNext:^(NSNumber * _Nullable x) {
        ...
        if ([x isEqualToNumber:@1]) {
            ...
            if ([self.state.state isEqualToNumber:@11]) {
                ...
                [self zx_setRightBtnWithText:@"处理" clickedBlock:^(ZXNavItemBtn * _Nonnull btn) {
                    @strongify(self);
                    [self handleBtnDisposeClicked:nil];
                }];
                
            }else if ([self.state.state isEqualToNumber:@0]) {
                
                [self zx_setRightBtnWithText:@"认定" clickedBlock:^(ZXNavItemBtn * _Nonnull btn) {
                    @strongify(self);
                    [self handleBtnAuditClicked:nil];
                }];
    ...
    [RACObserve(self.state, state) subscribeNext:^(NSNumber * _Nullable x) {
        ...
        if (self.accidentType == AccidentTypeFastAccident) {
            if ([x isEqualToNumber:@11]) {
                ...
                self.zx_navRightBtn.titleLabel.text = nil;
            }
        }else{
            self.zx_navRightBtn.titleLabel.text = nil;
        }
