...
#import "TaskFlowsForwardViewModel.h"

@implementation TaskFlowsForwardViewModel
    ...
    if (_command_commint == nil) {
        ...
        _command_commint = [[RACCommand alloc] initWithSignalBlock:^RACSignal * _Nonnull(id  _Nullable input) {
            ...
            RACSignal * t_signal = [RACSignal createSignal:^RACDisposable * _Nullable(id<RACSubscriber>  _Nonnull subscriber) {
                ...
                param.receiveUserId = self.userId;
                param.replyContent = self.content;
                ...
                manger.param = param;
                [manger configLoadingTitle:@"转发"];
                [manger startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest * _Nonnull request) {
                    if (manger.responseModel.code == CODE_SUCCESS) {
                        [subscriber sendNext:@"转发成功"];
                        
                    }else{
                        [subscriber sendNext:@"转发失败"];
                    }
