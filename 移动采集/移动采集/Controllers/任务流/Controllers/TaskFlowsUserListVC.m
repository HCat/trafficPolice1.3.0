...
@implementation TaskFlowsUserListVC

- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"选择执法人";
    
    self.layout_tableView_top.constant = Height_NavBar;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView == _tableView) {
        NSMutableArray *t_arr = _arr_addressBook[indexPath.section];
        if (t_arr) {
            AddressBookModel * model = t_arr[indexPath.row];
            if (self.block) {
                self.block(model);
            }
        };
    }else{
        AddressBookModel * model = _arr_search[indexPath.row];
        if (self.block) {
            self.block(model);
        }
    }
...
- (void)dealloc{
    ...
    LxPrintf(@"TaskFlowsUserListVC dealloc");
