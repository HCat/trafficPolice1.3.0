...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (touchMonthIndex == currentMonth || [LTSCalendarAppearance share].isShowSingleWeek) {
        LTSCalendarCollectionCell *lastCell = (LTSCalendarCollectionCell*)[collectionView cellForItemAtIndexPath:self.selectedIndexPath];
    ...
    if ([LTSCalendarAppearance share].isShowSingleWeek) {
        ...
        self.selectedIndexPath = self.currentSelectedIndexPath;
...
- (void)getDateDatas{
    ...
    if (self.currentDate == nil) {
        ...
        self.selectedDate = [LTSCalendarAppearance share].defaultDate;
    }
...
- (void)setSelectedDate:(NSDate *)selectedDate{
    ...
    if (self.eventSource && [self.eventSource respondsToSelector:@selector(calendarDidSelectedDate:)]) {
        [self.eventSource calendarDidSelectedDate:self.selectedDate];
    }
...
- (NSArray *)getDaysOfWeek:(NSDate *)date
{
    ...
    for (NSInteger i=0; i<7; i++) {
        ...
        if ([self isEqual:currentDate other:self.selectedDate]) {
            if ([LTSCalendarAppearance share].isShowSingleWeek) {
                NSInteger row = beginWeekIndexPath.row/7*7;
                row += i;
                
                self.selectedIndexPath = [NSIndexPath indexPathForRow:row inSection:round(NUMBER_PAGES_LOADED / 2)];
            }
            item.isSelected = YES;
        }
