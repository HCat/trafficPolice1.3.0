                ...
                containerView:(UIView *)containerView
{
    ...
    if (self) {
        ...
        self.mainScreenBounds = CGRectMake(0,frame.origin.y+frame.size.height, SCREEN_WIDTH, SCREEN_HEIGHT- (frame.origin.y+frame.size.height));
        self.isShown = NO;
        self.items = items;
        
        // Init button as navigation title
        self.menuButton = [[UIButton alloc] initWithFrame:self.bounds];
        ...
        self.menuTitle = [[UILabel alloc] initWithFrame:self.bounds];
        self.menuTitle.text = title;
        self.menuTitle.textColor = DefaultTextColor;
        ...
        self.tableView = [[PFTableView alloc] initWithFrame:CGRectMake(self.mainScreenBounds.origin.x,
                                                                       ...
                                                                       self.mainScreenBounds.size.height)
                                                      items:items
...
- (void)showMenu
{
    ...
    NSLog(@"%d",self.tableContainerView.subviews.count);
    [self.tableContainerView insertSubview:self.backgroundView atIndex:1];
    [self.tableContainerView insertSubview:self.tableView atIndex:2];
    // Rotate arrow
    [self rotateArrow];
    ...
    self.tableView.frame = CGRectMake(self.tableView.frame.origin.x,
                                      self.mainScreenBounds.origin.y-(CGFloat)(self.items.count) * self.configuration.cellHeight - 30,
    ...
    [UIView animateWithDuration:self.configuration.animationDuration * 1.5f
         ...
         usingSpringWithDamping:.7
          initialSpringVelocity:.5
                     ...
                     animations:^{
                         self.tableView.frame = CGRectMake(self.tableView.frame.origin.x,
                                                           self.mainScreenBounds.origin.y-30,
                                                           self.tableView.frame.size.width,
                                                           self.tableView.frame.size.height);
    ...
    [UIView animateWithDuration:self.configuration.animationDuration * 1.5f
         ...
         usingSpringWithDamping:.7
          initialSpringVelocity:.5
                     ...
                     animations:^{
                         self.tableView.frame = CGRectMake(self.tableView.frame.origin.x,
                                                           self.mainScreenBounds.origin.y-10,
                                                           self.tableView.frame.size.width,
                                                           self.tableView.frame.size.height);
                         self.backgroundView.alpha = self.configuration.maskBackgroundOpacity;
