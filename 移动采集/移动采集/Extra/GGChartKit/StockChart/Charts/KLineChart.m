...
@property (nonatomic, readwrite) NSString * volumIndexIndexName;
@property (nonatomic, readwrite) NSString * kLineIndexIndexName;
@property (nonatomic, readwrite) NSArray <id <KLineAbstract, VolumeAbstract, QueryViewAbstract> > * kLineArray;    ///< k线数组
...
- (void)updateVolumIndexLayer:(NSInteger)index
{
    ...
    runMainThreadWithBlock(^{
        @strongify(self);
        [self.volumIndexLayer removeFromSuperlayer];
        
        self.volumIndexIndexName = [KLineChart kVolumIndexLayerClazzName][index];
        Class clazz = NSClassFromString([self.volumIndexIndexName stringByAppendingString:@"Layer"]);
        
        self.volumIndexLayer = [[clazz alloc] init];
        self.volumIndexLayer.frame = self.redVolumLayer.frame;
        [self.volumIndexLayer setKLineArray:self.kLineArray];
        self.volumIndexLayer.currentKLineWidth = self.kLineScaler.shapeWidth;
        [self.scrollView.layer addSublayer:self.volumIndexLayer];
        [self updateSubLayer];
        
        if (self.indexChangeBlk) { self.indexChangeBlk([KLineChart kVolumIndexLayerClazzName][index]); }
    });
}
...
- (void)updateKLineIndexLayer:(NSInteger)index
{
    ...
    runMainThreadWithBlock(^{
        @strongify(self);
        [self.kLineIndexLayer removeFromSuperlayer];
        
        self.kLineIndexIndexName = [KLineChart kLineIndexLayerClazzName][index];
        Class clazz = NSClassFromString([self.kLineIndexIndexName stringByAppendingString:@"Layer"]);
        
        self.kLineIndexLayer = [[clazz alloc] init];
        self.kLineIndexLayer.frame = self.redLineLayer.frame;
        self.kLineIndexLayer.gg_width = self.kLineScaler.contentSize.width;
        [self.kLineIndexLayer setKLineArray:self.kLineArray];
        self.kLineIndexLayer.currentKLineWidth = self.kLineScaler.shapeWidth;
        [self.scrollView.layer addSublayer:self.kLineIndexLayer];
        [self updateSubLayer];
        
        if (self.indexChangeBlk) { self.indexChangeBlk([KLineChart kLineIndexLayerClazzName][index]); }
    });
}
