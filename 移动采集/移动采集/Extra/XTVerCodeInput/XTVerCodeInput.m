...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        /// 配置颜色
        _normalColor = XTUIColorFromRGB(0xffffff);
        _hlColor = XTUIColorFromRGB(0xffffff);
        _inputT = 4;
        self.setFrame = frame;
    }
    return self;
...
- (void)initSubviews {
    ...
    if (_inputT == 4) {
        _textView.frame = CGRectMake((WIDTH - (4 * K_W) - (3 * PADDING)) / 2, 0, (4 * K_W) + (3 * PADDING), 40);
    }else {
        _textView.frame = CGRectMake(10, 0, WIDTH - 20, 40);
    }
    /// 默认编辑第一个.
    //[self beginEdit];
    /// 初始化一个输入框
    ...
    for (int i = 0; i < _inputT; i ++) {
        ...
        label.textColor = XTUIColorFromRGB(0x333333);
        label.layer.borderWidth  = 1.f;
        label.layer.borderColor = [XTUIColorFromRGB(0x3396fc) CGColor];
        [subView addSubview:label];
        
        /// 4 光标
        UIBezierPath *path = [UIBezierPath bezierPathWithRect:CGRectMake(K_W / 2, 10, 2, K_H - 20)];
        CAShapeLayer *line = [CAShapeLayer layer];
        line.path = path.CGPath;
        line.fillColor =  XTUIColorFromRGB(0x3396fc).CGColor;
