//
//  LREmptyDataSetGeneralHandler.h
//  框架
//
...
@interface LREmptyDataSetGeneralHandler : LREmptyDataSetHandler
...
- (void)setTapViewHandler:(void (^)(UIView *view))tapViewHandler forState:(LRDataLoadState)state;

/** 设置state状态下的tapButtonHandler回调 */
- (void)setTapButtonHandler:(void (^)(UIButton *button))tapButtonHandler forState:(LRDataLoadState)state;
