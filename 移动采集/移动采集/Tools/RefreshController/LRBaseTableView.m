...
- (void)configTableView{
    ...
    [[[NSNotificationCenter defaultCenter] rac_addObserverForName:NOTIFICATION_HAVENETWORK_SUCCESS object:nil] subscribeNext:^(NSNotification * _Nullable x) {
        @strongify(self);
        [self networkStatusChange:YES];
        
    }];
    
    [[[NSNotificationCenter defaultCenter] rac_addObserverForName:NOTIFICATION_NONETWORK_SUCCESS object:nil] subscribeNext:^(NSNotification * _Nullable x) {
        @strongify(self);
        [self networkStatusChange:NO];
...
// 监听网络变化
- (void)networkStatusChange:(BOOL)obj{
    ...
    if (obj) {
        ...
        UIEdgeInsets edegeInset = self.contentInset;
        [UIView animateWithDuration:0.25 animations:^{
            self.contentInset = UIEdgeInsetsMake(0, edegeInset.left, edegeInset.bottom, edegeInset.right);
        }];
        
    }else{
        
        self.mj_header.ignoredScrollViewContentInsetTop = 40;
        self.networkView.hidden = NO;
        
        UIEdgeInsets edegeInset = self.contentInset;
        [UIView animateWithDuration:0.25 animations:^{
            self.contentInset = UIEdgeInsetsMake(40, edegeInset.left, edegeInset.bottom, edegeInset.right);
        }];
...
- (void)makeDefaultNetworkStatusView{
    ...
    self.networkView.frame = CGRectMake(0, -40, SCREEN_WIDTH, 40);
...
//    [handler setImage:bgImage controlState:UIControlStateHighlighted forState:LRDataLoadStateFailed];
    ...
    [handler setTapButtonHandler:^(UIButton *button) {
        if (self.tableViewPlaceholderBlock) {
            self.tableViewPlaceholderBlock();
        }
